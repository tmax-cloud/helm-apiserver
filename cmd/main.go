...
func main() {
	...
	router.HandleFunc(nsPrefix+releasePrefix, hcm.GetReleases).Methods("GET") // 설치된 release list 반환 (path-variable : 특정 release 정보 반환) helm client deployed releaselist 활용
	router.HandleFunc(nsPrefix+releasePrefix+"/{release-name}", hcm.GetReleases).Methods("GET")
	router.HandleFunc(nsPrefix+releasePrefix, hcm.InstallRelease).Methods("POST")                       // helm release 생성
	router.HandleFunc(nsPrefix+releasePrefix+"/{release-name}", hcm.UnInstallRelease).Methods("DELETE") // 설치된 release 전부 삭제 (path-variable : 특정 release 삭제)
	router.HandleFunc(nsPrefix+releasePrefix+"/{release-name}", hcm.RollbackRelease).Methods("PATCH")   // 일단 미사용 (update / rollback)
